...
- Dashboard: API-backed overview metrics for vehicles, online/offline state, alerts, companies, fleets, devices, and offline devices.
...
Feature services/types now exist for auth, companies (supporting company selection), fleets, vehicles, drivers, devices, tracking, geofences, alert rules, alerts, reports, and dashboard overview. New frontend domains should follow the same service/type/component pattern and use the shared `apiRequest()` helper.
